...
float	translateChecker = 0;
float	translateDirection = 0.1;
float	test = -.1;
bool	Distort;
...
Display( )
{
	...
	//Light Source 1 *Near Pyramid *PointLight *Yellow *Stationary
	//glEnable(GL_LIGHT0);
	SetPointLight(GL_LIGHT0, 0, 4, 4, 1, 1, 0.);
	glPushMatrix();
		SetMaterial(1., 1., 0., 1.);
		glColor3f(1., 1., 0.);
		glTranslatef(0., 3, 3.);
	...
	{
		glEnable(GL_LIGHT0);
		fprintf(stderr, "LIGHT0 ON\n");
	}
	else
	{
		glDisable(GL_LIGHT0);
		fprintf(stderr, "LIGHT0 OFF\n");
	}

	//Light Source 2 *Moving *PointLight *White *Moving With Box
	//glEnable(GL_LIGHT1);
	SetPointLight(GL_LIGHT1, 0, translateChecker - 1.5, 0, 1, 1, 1.);
	glPushMatrix();
		SetMaterial(1., 1., 1., 0.);
		glTranslatef(0., translateChecker - 1, 0.0);
		MjbSphere(.2, 60, 60);
	glPopMatrix();
	...
	{
		glEnable(GL_LIGHT1);
		fprintf(stderr, "LIGHT1 ON\n");
	}
	else
	{
		glDisable(GL_LIGHT1);
		fprintf(stderr, "LIGHT1 OFF\n");
	}

	//Light Source 3 *Stationary *SpotLight *Colored
	SetSpotLight(GL_LIGHT2, 0, -9.9, 0, 0.,3.,3., 1., 0., 0.);
	glPushMatrix();
		SetMaterial(1., 0., 0., 0.);
		glTranslatef(0., -10., 0.);
		MjbSphere(.2, 60, 60);
	...
	{
		glEnable(GL_LIGHT2);
		fprintf(stderr, "LIGHT2 ON\n");
	}
	else
	{
		glDisable(GL_LIGHT2);
		fprintf(stderr, "LIGHT2 OFF\n");
	}



	//TriangleList at the top
	glPushMatrix();
		SetMaterial(1., 0., 0., 1.);
		glTranslatef(0., 3., 3.);
	...
	glPushMatrix();
		SetMaterial(0., 0., 1., 1.);
		glTranslatef(0.,translateChecker, 0.0);
		glCallList(BoxList);
	glPopMatrix();
...
MjbSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (Distort)
			{
				//printf("%f", 5. * Time * M_PI);
				p->s = (lng + M_PI) / (5. * Time * M_PI);
				p->t = (lat + M_PI / 5. * Time) / M_PI;
			}
			else
			{
				p->s = (lng + M_PI) / (2. * M_PI);
