...
void	Resize( int, int );
void	Visibility( int );
void	DrawCurve(struct Curve*, struct Point);
...
Animate( )
{
	...
	ms %= msPerCycle;
	Time = (float)ms / (float)(msPerCycle - 1);
	Time = Time * 360.;
	Time = cos(Time * 3.14 / 180);
	printf("%f\n", Time);
...
Display( )
{
	...
	float TA = 2.5 * Time - 2.5; //0 to -5
	float TA2 = -2.5 * Time + 2.5; //0 to 5

	struct Curve curve;
	struct Point p0 = { 0., 0., 0. };
	struct Point p1 = { TA, -10., 4. };
	struct Point p2 = { -10., TA, 4. };
	struct Point p3 = { 0., 0., 0. };
...
InitMenus( )
{
	...
	int curvesmenu = glutCreateMenu(DoCurvesMenu);
	glutAddMenuEntry("Off", 0);
	glutAddMenuEntry("On", 1);
	...
	glutAddSubMenu("Curves", curvesmenu);
...
void DrawCurve(struct Curve* curve, struct Point p)
{
	glLineWidth(3.);
